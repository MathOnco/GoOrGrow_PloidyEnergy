...
vec g(vec E, vec u, vec v, double k, double R0, double R);       // Growth term
...
double &skewness, double &maxInfilt);
...
int main(int argc, char* argv[])
{
    ...
    for(int n=0;n<Nt-1;n++)
    {
        ...
        f0 = f(E.col(n),xi_u) ;
        g0 = g(E.col(n),u.col(n),v.col(n),ku, R0, R);
...
//        int idx_s = round(Nr*R0/R);
//        soln = solve(LHS.submat(0,0, idx_s, idx_s), RHS.submat(0,0, idx_s, idx_s)*u.col(n).subvec(0,idx_s));
        ...
        f0 = f(E.col(n),xi_v);
        g0 = g(E.col(n),u.col(n),v.col(n),kv, R0, R);
    ...
    double uMedian, uMean, uVar, uSkew, uInfilt,
    vMedian, vMean, vVar, vSkew, vInfilt;
    ...
    if(!fileExists)
    {
        ...
        "k_v\tu_{tot}\tv_{tot}\tu_{max}\tv_{max}\tu_{med}\tv_{med}\t" <<
        "u_{mean}\tv_{mean}\t" <<
    ...
    << "\t"<< vTot << "\t" << uInfilt << "\t"<< vInfilt << "\t" <<
    uMedian << "\t" << vMedian << "\t" << uMean << "\t" << vMean <<
    "\t" << uVar << "\t" << vVar << "\t" << uSkew << "\t" << vSkew << "\t" <<
    energyLeft << "\t" << steadyState << endl;
...
/*
    ...
    This function outputs the vector g(E, u, v,k, R0, R) - the growth term
...
*/
vec g(vec E, vec u, vec v, double k, double R0, double R)
{
	// Carrying capacity as a function of proximity to R0
	vec c = u;
    for(int ir=0;ir<u.n_elem;ir++)
    {
    	c(ir) = 1;
    	if(R * ir/u.n_elem > R0){
    		c(ir) = 1e-12;
...
double &skewness, double &maxInfilt)
{
    ...
    // skewness does not seem to agree with analytics as it currently seems
    // to depend on R0 when the distribution of cells stays roughly uniform.
    ...
    if(x0>0)
        maxInfilt = as_scalar(r(find(pdf>1e-4,1,"last")));
    else
        maxInfilt = 0.0;
